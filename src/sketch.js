const sketch = (p) => {
  ...
  const initGameSizeSlider = () => {
    if (playerSelect.value() == HMC)
      gameSizeSlider = p.createSlider(
        SIZE_MIN,
        SIZE_MAX_HAMILTONIAN,
        SIZE_INITIAL_HAMILTONIAN,
        SIZE_STEP
      );
    else
      gameSizeSlider = p.createSlider(
  ...
  const initGameSpeedSlider = () => {
    gameSpeedSlider = p.createSlider(
      FRAMERATE_MIN,
      FRAMERATE_MAX,
      FRAMERATE_INITIAL,
      FRAMERATE_STEP
  ...
  p.draw = () => {
    ...
    if (playerSelect.value() == HMC && !hmcModel.isHamiltonianCycle)
      p5Hamiltonian.notice(p, p.width / 2, p.height / 2);
  };
...
const p5Utils = {
  drawArrow: (p, direction, toX, toY) => {
    if (direction.eq(game.NORTH))
      p5Utils._drawArrow(p, node, 0, -0.5, 0.15, 0, toX, toY);
    else if (direction.eq(game.SOUTH))
      p5Utils._drawArrow(p, node, 0, 0.5, 0.15, 0, toX, toY);
    else if (direction.eq(game.EAST))
      p5Utils._drawArrow(p, node, 0.5, 0, 0, 0.15, toX, toY);
    else if (direction.eq(game.WEST))
      p5Utils._drawArrow(p, node, -0.5, 0, 0, 0.15, toX, toY);
  },
    ...
    model.policy.map((v, i) =>
      v.map((q, j) => {
        ...
        if (!model.allQEq(node)) {
          p.fill(
            `rgba(0,191,255,${Math.max(
              0.1,
              p.map(model.maxQ(node), vmax / 6, vmax, 0, 1)
            )})`
          );
          p5Utils.drawArrow(p, model.getAction(node), toX, toY);
        }
