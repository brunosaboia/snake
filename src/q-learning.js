...
const QLearn = (
  ...
  (ql.initPolicy = (nx, ny) => {
    ...
    (ql.getActionIndex = (node) =>
      ql
        .getQs(node)
        .reduce((acc, v, i, arr) => (i > 0 ? (v > arr[acc] ? i : acc) : i), 0)),
    /**
     * Returns a random action index
     */
    (ql.getRandomActionIndex = () =>
      utils.randInt(0, game.DIRECTIONS.length - 1)),
    /**
   ...
   */
  (ql.allQEq = (node) =>
    ql.getQs(node).every((q, i, arr) => (i > 0 ? q == arr[i - 1] : true))),
    /**
     * Returns the maximum Q at a given position
     */
    (ql.maxQ = (node) =>
      ql.getQs(node).reduce((acc, v) => (acc > v ? acc : v))),
    /**
    ...
    (ql.update = (next, state) => {
      ...
      if (ql.policy == null || state.justEaten || !ql.cumulativePolicy)
        ql.policy = ql.initPolicy(state.nx, state.ny);
      ...
      for (let ep = 0; ep < ql.nEpisodes; ep++) {
        ...
        for (let step = 0; step < maxSteps; step++) {
          ...
          const ai = ql.isExplore(head, exploreRate)
            ? ql.getRandomActionIndex()
            : ql.getActionIndex(head);
   ...
   */
  ql.isExplore = (node, exploreRate) =>
    Math.random() < exploreRate || ql.allQEq(node);
