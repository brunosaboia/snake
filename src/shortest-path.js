...
const ShortestPath = () => {
    ...
    const neighbors = [state.direction]
      ...
      .filter(
        (n) =>
          ...
          !state.snake.some((n1) => n1.eq(n) && !state.walls.has(n.x, n.y))
      );

    if (neighbors.length > 0)
      return fnShortestPath.buildPath([start, neighbors[0]]);
    else
      return fnShortestPath.buildPath([
        start,
        state.snake[0].sum(state.direction),
      ]);
  };
  ...
  sp.update = (state, goal) => {
    ...
    let path = fnShortestPath.search(
      state.snake[0],
      goal,
      state.nx,
      state.ny,
...
const fnShortestPath = {
  search: (
    ...
    while (open.length > 0) {
      ...
      if (current[1].eq(goal)) return fnShortestPath.buildPath(current);
        ...
        const currentDir =
          current[0] == null ? startDir : current[1].sub(current[0][1]);
        const dirs = [currentDir].concat(
          game.DIRECTIONS.filter((d) => !d.eq(currentDir))
        );
        dirs
  ...
  buildPolicy: (path, nx, ny) => {
    let policy = Array.from(Array(nx), (_) =>
      Array.from(Array(ny), (_) => null)
    );
