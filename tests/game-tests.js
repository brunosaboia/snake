const gameTests = {
  ...
  nextSnake: () => {
    ...
    eq(
      true,
      game
        .nextSnake(snake1, Node(1, 1), true)
        .every((node, i) => node.eq(snake2[i]))
    );
    eq(false, game.nextSnake(snake1, Node(1, 1), true).length == snake3.length);
    eq(
      false,
      game.nextSnake(snake1, Node(1, 1), false).length == snake2.length
    );
    eq(
      true,
      game
        .nextSnake(snake1, Node(1, 1), false)
        .every((node, i) => node.eq(snake3[i]))
    );
  },
